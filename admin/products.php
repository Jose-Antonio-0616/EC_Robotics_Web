...
if (isset($_POST['agregar_producto'])) {
    ...
    if ($select_products->rowCount() > 0) {
        ...
        $insert_products = $conn->prepare("INSERT INTO `productos`(nombre, detalles, precio, imagen_01, imagen_02, imagen_03) VALUES(?,?,?,?,?,?)");
...
if (isset($_GET['borrar'])) {
    ...
    $delete_cart = $conn->prepare("DELETE FROM `carro` WHERE pid = ?");
    $delete_cart->execute([$delete_id]);
    $delete_wishlist = $conn->prepare("DELETE FROM `lista_de_deseos` WHERE pid = ?");
...
<body>
    ...
    <section class="add-products">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="flex">
                <div class="inputBox">
                    <span>nombre de producto (requerido)</span>
                    <input type="text" class="box" required maxlength="100" placeholder="ingrese el nombre del producto"
                        name="nombre">
                </div>
                <div class="inputBox">
                    ...
                    <input type="number" min="0" class="box" required max="9999999999"
                        ...
                        onkeypress="if(this.value.length == 10) return false;" name="precio">
                </div>
                <div class="inputBox">
                    <span>imagen 01 (requerida)</span>
                    <input type="file" name="imagen_01" accept="image/jpg, image/jpeg, image/png, image/webp"
                        class="box" required>
                </div>
                <div class="inputBox">
                    <span>imagen 02 (requerido)</span>
                    <input type="file" name="imagen_02" accept="image/jpg, image/jpeg, image/png, image/webp"
                        class="box" required>
                </div>
                <div class="inputBox">
                    <span>imagen 03 (requerido)</span>
                    <input type="file" name="imagen_03" accept="image/jpg, image/jpeg, image/png, image/webp"
                        class="box" required>
                </div>
                <div class="inputBox">
                    <span>detalles del producto (requerido)</span>
                    <textarea name="detalles" placeholder="ingrese los detalles del producto" class="box" required
                        maxlength="500" cols="30" rows="10"></textarea>
                </div>
            </div>

            <input type="submit" value="agregar producto" class="btn" name="agregar_producto">
        </form>
    ...
    <section class="show-products">
        ...
        <h1 class="heading">producto agregado!</h1>
